...
int     main(int ac, char **av)
{
    ...
    std::string         uport(av[1]);
    std::string         paswd(av[2]);
    int                 port = FromString<int>(uport);
    the_serv            server;
    ...
    server.password = paswd + "/r/n";
    while (++i < 30)
        client_socket[i] = 0;
    if( (master_socket = socket(AF_INET , SOCK_STREAM , 0)) == 0)//init master_socket
    {
        perror("socket failed");
        exit(EXIT_FAILURE);
    }
    if(setsockopt(master_socket, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, sizeof(opt)) < 0 )//init multi-connection handle from master socket
    {
        perror("setsockopt");
        exit(EXIT_FAILURE);
    }
    ...
    printf("master socket : %d |readfds[0] : %ld\n", master_socket, readfds.fds_bits[0]);
    while(1)
    {
        FD_ZERO(&readfds);
        ...
        if (FD_ISSET(master_socket, &readfds))
        {
            ...
            send(new_socket, "Nouvelle connection\n", (ssize_t)strlen("Nouvelle connection\n"), 0);
        }
        else{
            printf("Une commande autre\n");
        }
    }
}
//FD_SET VARIABLE :
